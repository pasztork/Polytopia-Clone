...
{
    public static string MapFilePath { private get; set; } = string.Empty;
    ...
    {
        if (s_webSockets.Count > PlayerCount || s_webSockets.Contains(webSocket))
        {
            return false;
        }
    ...
    public static void CreatePlayer(string playerJson, WebSocket webSocket)
    {
        ...
        if (s_playerToWebSocketsDict.Count == PlayerCount)
        {
            StartGame();
        }
    }
